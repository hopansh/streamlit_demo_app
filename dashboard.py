...
df_time_diff = pd.merge(df_time, df_agg.loc[:, [
                        'Video', 'Video publish time']], left_on='External Video ID', right_on='Video')
df_time_diff['days_published'] = (
    df_time_diff['Date'] - df_time_diff['Video publish time']).dt.days
...
views_days.columns = ['days_published', 'mean_views',
...
views_cumulative.loc[:, ['median_views', '80pct_views', '20pct_views']] = views_cumulative.loc[:, [
...
st.header("Youtube Analysis Dashboard")
st.write("Author: *Hopansh Gahlot!*")
...
if add_sidebar == 'Aggregate Metrics':
    ...
    st.dataframe(df_agg_diff_final.style.hide().applymap(style_negative, props='color:red;').applymap(
...
if add_sidebar == 'Individual Video Analysis':
    ...
    agg_time_filtered = df_time_diff[df_time_diff['Video Title']
                                     == video_select]
    first_30 = agg_time_filtered[agg_time_filtered['days_published'].between(
        0, 30)]
